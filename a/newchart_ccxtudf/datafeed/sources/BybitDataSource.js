...
class BybitFuturesDataSource extends BaseDataSource {
    constructor(config = {}) {
        super(config);
        this.name = 'BybitFuturesDataSource';
    ...
    async resolveSymbol(symbolName, onResolve, onError) {
        try {
            ...
            const response = await fetch(`${this.apiUrl}/v5/market/instruments-info?category=linear&symbol=${symbol}`);
            ...
            console.error('[BybitFuturesDataSource] Resolve error:', error);
    ...
    async getBars(symbolInfo, resolution, periodParams, onResult, onError) {
        try {
            ...
            const url = `${this.apiUrl}/v5/market/kline?category=linear&symbol=${symbol}&interval=${interval}&start=${periodParams.from * 1000}&end=${periodParams.to * 1000}&limit=1000`;
            ...
            console.error('[BybitFuturesDataSource] GetBars error:', error);
            onError(error.message);
        }
    }
    ...
    async subscribeBars(symbolInfo, resolution, onTick, listenerGuid, onResetCacheNeededCallback) {
        ...
        ws.onerror = (error) => {
            console.error('[BybitFuturesDataSource] WebSocket error:', error);
        };
...
export default BybitFuturesDataSource;
