 ...
 * OKX Spot DataSource
 */
class OKXSpotDataSource extends BaseDataSource {
    constructor(config = {}) {
        super(config);
        this.name = 'OKXSpotDataSource';
    ...
    async resolveSymbol(symbolName, onResolve, onError) {
        try {
            ...
            const response = await fetch(`${this.apiUrl}/api/v5/public/instruments?instType=SPOT&instId=${instId}`);
            ...
            console.error('[OKXSpotDataSource] Resolve error:', error);
    ...
    async getBars(symbolInfo, resolution, periodParams, onResult, onError) {
        try {
            ...
            console.error('[OKXSpotDataSource] GetBars error:', error);
            onError(error.message);
        }
    }
    ...
    async subscribeBars(symbolInfo, resolution, onTick, listenerGuid, onResetCacheNeededCallback) {
        ...
        ws.onerror = (error) => {
            console.error('[OKXSpotDataSource] WebSocket error:', error);
        };
...
export default OKXSpotDataSource;
