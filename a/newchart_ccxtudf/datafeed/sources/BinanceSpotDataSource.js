...
class BinanceSpotDataSource extends BaseDataSource {
    constructor(config = {}) {
        super(config);
        this.name = 'BinanceSpotDataSource';
    ...
    async onReady() {
        return {
            ...
            supports_time: true,
            exchanges: [
                { value: 'BINANCE', name: 'Binance Spot', desc: 'Binance Spot' }
            ]
        };
    }
    ...
    async searchSymbols(userInput, exchange, symbolType, onResult) {
        ...
        try {
            ...
            console.error('[BinanceSpotDataSource] Search error:', error);
            onResult([]);
        }
    }
    ...
    async resolveSymbol(symbolName, onResolve, onError) {
        try {
            ...
            console.error('[BinanceSpotDataSource] Resolve error:', error);
    ...
    async getBars(symbolInfo, resolution, periodParams, onResult, onError) {
        try {
            ...
            const url = `${this.apiUrl}/api/v3/klines?symbol=${symbol}&interval=${interval}&startTime=${periodParams.from * 1000}&endTime=${periodParams.to * 1000}&limit=1000`;
            ...
            console.error('[BinanceSpotDataSource] GetBars error:', error);
            onError(error.message);
        }
    }
    ...
    async subscribeBars(symbolInfo, resolution, onTick, listenerGuid, onResetCacheNeededCallback) {
        ...
        ws.onerror = (error) => {
            console.error('[BinanceSpotDataSource] WebSocket error:', error);
        };
...
export default BinanceSpotDataSource;
