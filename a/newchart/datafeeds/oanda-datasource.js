...
class OANDADatasource extends BaseDatasource {
    ...
    async resolveSymbol(symbolName) {
        ...
        try {
            return {
                ...
                description: symbol,
                type: OANDA_CONFIG.type,
                session: '0000-2400:1234567', // 24/7 trading
                timezone: 'Etc/UTC',
                ticker: symbol,
                exchange: OANDA_CONFIG.exchange,
                minmov: 1,
                ...
                has_no_volume: true // Forex không có volume thực
            };
        } catch (error) {
    ...
    /**
     * Lấy 15000 bars (3 lần x 5000) và cache lại
     */
    async fetch15000Bars(symbol, oandaSymbol, resolution) {
        ...
        console.log(`${OANDA_CONFIG.logPrefix} Fetching 15000 bars for ${symbol} (${resolution})...`);

        try {
            const now = Math.floor(Date.now() / 1000);
            let allBars = [];

            // Lần 1: Lấy 5000 bars gần nhất
            const batch1 = await this.fetchBatch(oandaSymbol, granularity, now);
            if (batch1.length > 0) {
                allBars = batch1;
                console.log(`${OANDA_CONFIG.logPrefix} Batch 1: ${batch1.length} bars`);

                // Lần 2: Lấy 5000 bars tiếp theo
                const oldestTime1 = Math.floor(batch1[0].time / 1000);
                const batch2 = await this.fetchBatch(oandaSymbol, granularity, oldestTime1 - 1);
                if (batch2.length > 0) {
                    allBars = [...batch2, ...allBars];
                    console.log(`${OANDA_CONFIG.logPrefix} Batch 2: ${batch2.length} bars`);

                    // Lần 3: Lấy 5000 bars cuối cùng
                    const oldestTime2 = Math.floor(batch2[0].time / 1000);
                    const batch3 = await this.fetchBatch(oandaSymbol, granularity, oldestTime2 - 1);
                    if (batch3.length > 0) {
                        allBars = [...batch3, ...allBars];
                        console.log(`${OANDA_CONFIG.logPrefix} Batch 3: ${batch3.length} bars`);
                    }
                }
            }
            ...
            console.log(`${OANDA_CONFIG.logPrefix} Total unique bars: ${uniqueBars.length}`);
            ...
            console.error(`${OANDA_CONFIG.logPrefix} Error fetching 15000 bars:`, error);
            return [];
        }
    }
    ...
    async getBars(symbolInfo, resolution, periodParams) {
        ...
        try {
            ...
            // Lấy 15000 bars từ cache hoặc API
            const allBars = await this.fetch15000Bars(symbol, oandaSymbol, resolution);
