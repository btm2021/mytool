const BinanceFutureDatafeed = {
    ...
    async fetchSymbols() {
        ...
        this.symbolsCache = data.symbols
            ...
            .map(s => ({
                symbol: s.symbol,
                full_name: `Binance Futures:${s.symbol}`,
                description: `${s.baseAsset}/${s.quoteAsset} Perpetual`,
                exchange: 'Binance Futures',
                type: 'crypto',
                pricescale: Math.pow(10, s.pricePrecision),
                minmov: 1,
                ticker: s.symbol
            }));
    ...
    async getBars(symbolInfo, resolution, periodParams, onHistoryCallback, onErrorCallback) {
        try {
            ...
            const url = `${this.baseUrl}/fapi/v1/klines?symbol=${symbolInfo.ticker}&interval=${interval}&startTime=${from * 1000}&endTime=${to * 1000}&limit=1500`;
