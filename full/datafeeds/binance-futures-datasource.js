...
class BinanceFuturesDatasource extends BaseDatasource {
    ...
    calculatePrecision(price) {
        ...
        let decimals;
        // Giá lớn (>= 1000): làm tròn đến 1 số thập phân
        if (price >= 1000) {
            decimals = 1;
        }
        // Giá cực nhỏ (< 0.0001): làm tròn đến 8 số thập phân
        else if (price < 0.0001) {
            decimals = 8;
        }
        // Giá rất nhỏ (< 0.001): làm tròn đến 6 số thập phân
        else if (price < 0.001) {
            decimals = 6;
        }
        // Giá siêu nhỏ (< 0.01): làm tròn đến 5 số thập phân
        else if (price < 0.01) {
            decimals = 5;
        }
        // Giá trung bình: làm tròn đến 4 số thập phân
        else {
            decimals = 4;
        }
    ...
    async getBars(symbolInfo, resolution, periodParams) {
        ...
        try {
            ...
            console.error(`${BINANCE_FUTURES_CONFIG.logPrefix} Error fetching bars:`, error);
            throw error;
        }
    }
